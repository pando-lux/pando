...
const RATE_LIMITS: Record<string, { max: number; envVar: string; windowMs?: number }> = {
  ...
  'POST /council/request':           { max: 3,  envVar: 'PANDO_RATE_REPORT', windowMs: 3600_000 }, // legacy route → pando-infra team
...
export class ApiServer {
  ...
  private setupAuth(): void {
    ...
    this.fastify.addHook('onRequest', async (request: any, reply: any) => {
      ...
      // /council/* routes are legacy aliases for pando-infra team that also require auth.
