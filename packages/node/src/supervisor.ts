...
// KB: Read from teams .env (same source the teams server uses via dotenv).
if (!process.env.PANDO_API_KEY && TEAMS_PATH) {
  try {
    const envFile = readFileSync(join(TEAMS_PATH, '.env'), 'utf-8');
    const match = envFile.match(/^PANDO_API_KEY=(.+)$/m);
    if (match) process.env.PANDO_API_KEY = match[1].trim();
  } catch { /* .env not found — teams server will also fail */ }
}
