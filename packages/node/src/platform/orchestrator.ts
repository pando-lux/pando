...
export class Orchestrator {
  ...
  private buildBootPrompt(board: BoardState, agent: AgentIdentity): string {
    ...
    if (isQaUser) {
      ...
      sections.push('');
      sections.push('After commit_code + propose_upgrade, you MUST trigger a node restart before picking up the next task.');
      sections.push('Why: You and all agents run from the built code. Until the node restarts, everyone runs STALE code.');
      sections.push('Your decisions after a commit are based on old reality until the restart loads the new build.');
      sections.push('');
      sections.push('The upgrade pipeline:');
      sections.push('1. Worker finishes code → you commit_code (auto-triggers propose_upgrade)');
      sections.push('2. Governance checks run (build, scenarios, security). If they pass → proposal approved.');
      sections.push('3. You MUST push to origin BEFORE proposing so EC2 nodes can pull the code.');
      sections.push('   Run: spawn_worker role=devops task="git push origin master"');
      sections.push('4. After proposal passes, trigger restart: `curl -X POST http://127.0.0.1:4000/v1/upgrade`');
      sections.push('5. ONLY after restart: pick up next directive. Not before.');
      sections.push('');
      sections.push('NEVER stack multiple commits without pushing + proposing + restarting between them.');
      sections.push('Each fix should flow: commit → push → propose → restart → verify → next task.');
      sections.push('');
