...
export class Agent {
  ...
  buildClaudeMd(taskContext?: string): void {
    ...
    const identity = [
      ...
      'When your task is COMPLETE or FAILED, you MUST call this endpoint:',
      '```bash',
      `curl -X POST ${base}/agents/${this.id}/report \\`,
      apiToken ? `  -H "Authorization: Bearer ${apiToken}" \\` : '',
      '  -H "Content-Type: application/json" \\',
      '  -d \'{"status": "complete", "summary": "What you did and the result"}\'',
      '```',
      'Status options: `complete`, `failed`, `progress`',
      '',
      '### Message Parent',
      'Send a message to your parent agent for questions or progress updates:',
      '```bash',
