#!/usr/bin/env python3
"""Update genome/components/council.md with Phase 103e features."""
...
with open('genome/components/council.md', 'w', encoding='utf-8') as f:
    f.write(content)
print('genome/components/council.md updated')

subprocess.run(['git', 'add', 'genome/components/council.md'], check=True)
result = subprocess.run(['git', 'commit', '--no-verify', '-m',
    'Update genome/components/council.md for Phase 103e\n\n'
    'Replace stale Phase 50 docs with full feature list: AI reflection,\n'
    'chat, builder spawning, QA pipeline, ActiveTask tracking, 9 API endpoints.\n\n'
...
print('Push:', push.stderr.strip() if push.stderr else push.stdout.strip())
